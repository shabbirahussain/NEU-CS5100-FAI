...
public class AStarController extends Controller<MOVE> {
	private static final int MAX_DEPTH = 100; //Astar can support much deeper search.
	...
	public MOVE getMove(Game game, long timeDue) {
		...
		if(!USE_MOVE_MEMORY || this.moveList.isEmpty()){ // start new search
			...
			PacManNode solution = aStarSearch(pmNode, targetIndex, MAX_DEPTH);
	...
	/**
	 * Performs AStar search from pacman to target index.
	 ...
	 */
	private PacManNode aStarSearch(final PacManNode pmStartNode, final int targetIndex, final int maxdepth) {
		PriorityQueue<PacManNode> aStarQueue = new PriorityQueue<PacManNode>();
		aStarQueue.add(pmStartNode);		
		PacManNode pmNode = null;
		
		while (!aStarQueue.isEmpty()) {
			pmNode = aStarQueue.remove(); // Pop topmost node from queue.
			...
			if (pmNode.depth <= maxdepth) {
				...
				aStarQueue.addAll(pmNodeChildren);
				/*
				for(PacManNode pmChild : pmNodeChildren){
					// If path leads to ghost cut the branch, do not add it to queue.
					if (pmNode.game.wasPacManEaten())  continue;
					
					// Add next state to queue end.
					aStarQueue.add(pmChild);
				}//*/
			}
		}
		return pmNode;
	...
	private LinkedList<PacManNode> getAllChildren4Depth0(PacManNode pmNode){
		...
		for(MOVE m : availableMoves){
			...
			pmChildNode.updateMinimaxScore(SmartController.getHeuristicValue(pmNode, pmChildNode));
