 ...
 */
public class BreadthFirstController extends Controller<MOVE> {
	...
	public MOVE getMove(Game game, long timeDue) {
		...
		if(!USE_MOVE_MEMORY || this.moveList.isEmpty()){ // start new search
			...
			PacManNode solution = breadthFirstSearch(pmNode, targetIndex, MAX_DEPTH);
	...
	/**
	 * Performs breadth first search from pacman to target index.
	 ...
	 */
	private PacManNode breadthFirstSearch(final PacManNode pmStartNode, final int targetIndex, final int maxdepth) {
		Queue<PacManNode> bfsQueue = new LinkedList<PacManNode>();
		bfsQueue.add(pmStartNode);		
		PacManNode pmNode = null;

		while (!bfsQueue.isEmpty()) {
			pmNode = bfsQueue.remove(); // Pop topmost node from queue.
